...
//#endif
	...
	//Step Time_Serial Time_Parrallel Time_Parrallel_Improved Speecup_Parallel Speecup_Parallel_Improved Speedup_Gain
	printf("%-5s %14s %19s %26s %20s %29s %16s\n", "step","Serial_t(ms)", "Parrallel_t(ms)", "Parrallel_Improved_t(ms)", "Speedup_Parallel","Speedup_Parallel_Improved","Speedup_Gain");
	for(int step=100; step<=100; step+=100){
		for(int samples =0; samples <SAMPLES ; samples++){
					...
					//std::cout << '\n';
						...
						timeElapsed[1][samples] = time2;
						timeElapsed[2][samples] = time3;
...
#pragma omp parallel for  private(i,CASE) shared(average, variance)
  for( CASE=0; CASE<3; CASE++){
      ...
      //printf("sample needed CASE %d Step %d  parallel %lf Improved %lf ---> %f\n", CASE, step, average[0]/average[1],average[0]/average[2], pow(((100 * variance[CASE] * 1.96)/(5* average[CASE])),2));

  }
	float Speedup_Parallel =average[0]/average[1];
	float Speedup_Parallel_Improved = average[0]/average[2];
	printf("%-5d %14.5lf %19.5lf %26.5lf %20.5lf %29.5lf %16.5lf\n", step, average[0], average[1], average[2], Speedup_Parallel,Speedup_Parallel_Improved,(Speedup_Parallel_Improved - Speedup_Parallel));

  printf("Serial		%lf 	%lf: %lf\n", average[0], variance[0], sample(average[0], variance[0]));
	printf("Paralle		%lf 	%lf: %lf\n",   average[1], variance[1],sample(average[1], variance[1]));
	printf("Improved	%lf 	%lf: %lf\n\n",   average[2], variance[2],sample(average[2], variance[2]));
}
...
double sample(double mean, double variance){
	...
	return pow(((100 * variance * 1.96)/(5* mean)),2);
